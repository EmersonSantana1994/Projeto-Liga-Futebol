...
module.exports = {
    buscarTodos: () => {
        return new Promise((aceito, rejeitado)=>{
            db.query('SELECT * FROM ranking_jogadores ORDER BY pontos Desc', (error, results)=>{
    ...
    inserirPontos: (dados) => {
        return new Promise((aceito, rejeitado)=>{
            db.query('INSERT INTO ranking_jogadores (pontos, nome) VALUES (?,?)', [dados.pontos, dados.nome], (error, results) => {
                if(error) { 
                    rejeitado(error); 
                    return; }
                    aceito(results);
    ...
    buscarNome: (nome) => {
        return new Promise((aceito, rejeitado)=>{
            db.query('SELECT * FROM ranking_jogadores WHERE nome IN(?) ', [nome], (error, results) => {
                if(error) { rejeitado(error); return; }
               //vai verificar se retornou mais de 1 e pegar o 1
                    aceito(results);
            });
