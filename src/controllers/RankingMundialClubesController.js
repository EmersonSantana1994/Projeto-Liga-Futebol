...
const RankingMundialClubesModal = require('../services/RankingMundialClubesModal');
...
module.exports = {
    async buscarTodos(req, res) {
        ...
        let futebol = await RankingMundialClubesModal.buscarTodos();
            json.result = futebol
    ...
    async inserirPontos(req, res) {
        ...
        let dados = { pontos: req.body.pontos, nome: req.body.nome  }
            let inserir = await RankingMundialClubesModal.inserirPontos(dados);
            if (inserir) {
    ...
    async buscarNome(req, res) {
        ...
        let futebol = await RankingMundialClubesModal.buscarNome(format);
        if (futebol) {
            json.result = futebol; //se tiver nota ele joga no json
        }
    ...
    async atualizaPontos(req, res) {
        ...
        let dados = {id: req.body.id, pontos: req.body.pontos }
            let inserir = await RankingMundialClubesModal.atualizaPontos(dados);
            if (inserir) {  
                json.result = inserir; //se tiver nota ele joga no json
            }
            return res.json(json)
    ...
    async deletar(req, res) {
        let json = { error: '', result: {} };
            await RankingMundialClubesModal.deletar();
            return res.json(json)
    },
